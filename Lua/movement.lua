	...
	while (take <= takecount) or finaltake do
		...
		if (finaltake == false) then
			if (take == 1) then
				...
				if (featureindex["3d"] ~= nil) and (spritedata.values[CAMTARGET] ~= 0) and (spritedata.values[CAMTARGET] ~= 0.5) and
					(no3d == false) then
					local sleeping = false
					local domove = false
					local turndir = 0
			...
			if (take == 2) then
				...
				for a, v in ipairs(movers) do
					print(mmf.newObject(v).strings[UNITNAME] .. " (" .. v .. ") move")
				end
								...
								if (been_seen[bsid] == nil) then
									...
									if (fearname ~= "empty") then
										...
										table.insert(moving_units,
											{ unitid = uid, reason = "fear", state = 0, moves = amount, dir = feardir, xpos = x, ypos = y })
										...
										table.insert(moving_units,
											{ unitid = 2, reason = "fear", state = 0, moves = amount, dir = feardir, xpos = x, ypos = y })
										been_seen[bsid] = #moving_units
									end
								else
									...
									if (b ~= v) and floating(b, v, x, y) then
										...
										if (isstill_or_locked(b, x, y, unit.values[DIR]) == false) then
											if (been_seen[b] == nil) then
												table.insert(moving_units,
													{ unitid = b, reason = "shift", state = 0, moves = 1, dir = unit.values[DIR], xpos = x, ypos = y })
						...
						for a, unit in ipairs(units) do
							...
							if floating_level(unit.fixed) then
								...
								if (isstill_or_locked(unit.fixed, x, y, leveldir) == false) and (issleep(unit.fixed, x, y) == false) then
									table.insert(moving_units,
										{ unitid = unit.fixed, reason = "shift", state = 0, moves = 1, dir = unit.values[DIR], xpos = x, ypos = y })
								end
							end
						end
					end
			...
			for i, data in ipairs(still_moving) do
				if (data.unitid ~= 2) then
					...
					table.insert(moving_units,
						{ unitid = data.unitid, reason = data.reason, state = data.state, moves = data.moves, dir = unit.values[DIR],
							xpos = unit.values[XPOS], ypos = unit.values[YPOS] })
				else
					-- MF_alert("Still moving: " .. tostring(data.xpos) .. ", " .. tostring(data.ypos) .. ", " .. tostring(data.moves))
					table.insert(moving_units,
						{ unitid = data.unitid, reason = data.reason, state = data.state, moves = data.moves, dir = data.dir,
							xpos = data.xpos, ypos = data.ypos })
				end
			end
									...
									if (swap == nil) or still then
										...
										if (obs == 1) then
											...
											for f, g in pairs(specials) do
												...
												if (g[1] == thisobs) and ((g[2] == "weak") or (g[2] == "mine")) then
													--End of Change
													solid = false
													obs = 0
													result = math.max(result, 0)
									...
									if (state < 2) then
										...
										if (state < 3) then
											...
											if ((data.reason == "move") or (data.reason == "chill")) and (state < 4) then
												...
												local mine = hasfeature(name, "is", "mine", data.unitid, x, y)

												if (mine ~= nil) then
													local delthese = SB_do_boom(nil, data.unitid)
													handledels(delthese)

													if (issafe(data.unitid, x, y) == false) then
														delete(data.unitid, x, y)
														generaldata.values[SHAKE] = 3

														local pmult, sound = checkeffecthistory("boom")
														MF_particles("smoke", x, y, 5 * pmult, 0, 3, 1, 1)
														setsoundname("removal", 1, sound)
														data.moves = 1
													end
													--End of Adds
												elseif (weak ~= nil) and (issafe(data.unitid, x, y) == false) then
													delete(data.unitid, x, y)
													generaldata.values[SHAKE] = 3
...
function check(unitid, x, y, dir, pulling_, reason)
		...
		for i, id in ipairs(obs) do
			if (id == -1) then
				...
				if (lockpartner ~= "") and (pulling == false) then
					...
					if (partner ~= nil) and ((issafe(id, x + ox, y + oy) == false) or (issafe(unitid, x, y) == false)) and
						floating(id, unitid, x + ox, y + oy) then
						valid = false
				...
				--Added by slimebreath6078
				local mine = hasfeature(obsname, "is", "mine", id, x + ox, y + oy)
				if (mine ~= nil) and (pulling == false) then
					if (issafe(id, x + ox, y + oy) == false) and floating(id, unitid, x + ox, y + oy) then
						--valid = false
						table.insert(specials, { id, "mine" })
					end
				end
				--End of Adds
				...
				if valid then
					...
					if (
						((isstop ~= nil) and (ispush == nil) and ((ispull == nil) or ((ispull ~= nil) and (pulling == false)))) or
							((ispull ~= nil) and (pulling == false) and (ispush == nil))) and (isswap == nil) then
	...
	elseif (phantom == nil) then
		...
		if (lockpartner ~= "") and (pulling == false) then
			...
			if (partner ~= nil) and ((issafe(2, x + ox, y + oy) == false) or (issafe(unitid, x, y) == false)) and
				floating(unitid, 2, x + ox, y + oy) then
				valid = false
		...
		--Added by slimebreath6078
		local mine = hasfeature("empty", "is", "mine", 2, x + ox, y + oy)
		if (mine ~= nil) and (pulling == false) then
			if (issafe(2, x + ox, y + oy) == false) and floating(unitid, 2, x + ox, y + oy) then
				--valid = false
				table.insert(specials, { 2, "mine" })
			end
		end
		--End of Adds
...
function trypush(unitid, ox, oy, dir, pulling_, x_, y_, reason, pusherid)
	...
	--Added by slimebreath6078
	local mine = hasfeature(name, "is", "mine", unitid, x_, y_)
	--End of Adds

	--Changed by slimebreath6078
	if ((weak == nil) and (mine == nil)) or pulling or (((weak ~= nil) or (mine ~= nil)) and issafe(unitid, x_, y_)) then
		--End of Adds
		local result = 0
	...
	if (HACK_MOVES < 10000) then
		...
		while (finaldone == false) and (HACK_MOVES < 10000) do
			...
			elseif (result == 2) then
				...
				--Added by slimebreath6078
				if (mine ~= nil) then
					local delthese = SB_do_boom(nil, unitid)
					handledels(delthese)
	...
	for i, v in pairs(specials) do
		if (gone == false) then
			...
			if (b ~= 2) and (deleted[b] ~= nil) then
				...
				dodge = true
				--Changed by slimebreath6078
			elseif (b ~= 2) and (reason ~= "weak") and (reason ~= "mine") then
				--End of Changes
			...
			if (dodge == false) then
				...
				elseif (reason == "weak") then
					...
					--Added by slimebreath6078
				elseif (reason == "mine") then
					if (b == 2) and (unitid ~= 2) then
						local delthese = SB_do_boom(nil, b)
						handledels(delthese)

						local pmult, sound = checkeffecthistory("boom")
						MF_particles("smoke", bx, by, 5 * pmult, 0, 3, 1, 1)
						generaldata.values[SHAKE] = 3
						delete(b, bx, by)
