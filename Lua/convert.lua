function conversion(dolevels_)
	...
	for i, v in pairs(features) do
		...
		if (operator == "is") or (operator == "write") or (operator == "become") then
			...
			if (getmat(thing) ~= nil) or (thing == "not " .. name) or (thing == "all") or (unitreference[thing] ~= nil) or
				((thing == "text") and (unitreference["text_text"] ~= nil)) or (thing == "revert") or
				((operator == "write") and getmat_text("text_" .. name)) or
				--Add by slimebreath6078--
				((operator == "write") and (string.sub(name, 0, 5) == "text_")) or
				(string.sub(thing, 0, 5) == "text_") or
				(string.sub(name, 0, 5) == "text_" and thing == "not text")
			--End of Add--
			then
						...
						if (verb == "is") or (verb == "become") then
							if (target == name) and (object ~= "word") and ((object ~= name) or (verb == "become")) then
								if (object ~= "text") and (object ~= "revert") then
									--Add by slimebreath6078 --
									if (object == "not " .. name) or ((string.sub(name, 0, 5) == "text_") and (object == "not text")) then
										--End of Add--
										...
										if (inserted == false) and (string.sub(object, 0, 5) == "text_") then
											table.insert(output, { object, conds, "is" })
										end
									end
				...
				if (#output > 0) then
					...
					for k, v3 in pairs(output) do
						...
						if (op == "is") then
							...
							elseif (string.sub(object, 0, 5) == "text_") then
								table.insert(conversions, v3)
							...
							elseif (object == "text") then
								if (string.sub(name, 0, 5) ~= "text_") then
				...
				for i, unitid in pairs(targets) do
					...
					if (unit.flags[CONVERTED] == false) then
						for a, matdata in pairs(mats) do
							...
							if ((SB_samename_object(unit.strings[UNITNAME], mat1) == true) or (SB_is_group_of_text(unit) == true)) and
								(SB_samename_object(mat1, mat2) == false) then
...
function doconvert(data, extrarule_)
	...
	if (unitid ~= 2) then
		...
		x, y, dir, name, id, completed, ogname = unit.values[XPOS], unit.values[YPOS], unit.values[DIR],
			unit.strings[UNITNAME
			], unit.values[ID], unit.values[COMPLETED], unit.originalname
	end
	...
	if (style == "convert") then
		for a, mats2data in ipairs(mats2) do
			...
			if (mat2 ~= "empty") and (mat2 ~= "error") and (mat2 ~= "revert") and (mat2 ~= "createall") then
				...
				if (spritedata.values[VISION] == 0) or
					((newunit.values[TILING] == 1) and (newunit.values[ZLAYER] <= 10) and (newunit.values[ZLAYER] >= 0)) then
					dynamic(newunitid)
				end
		...
		if delthis and (unit.flags[DEAD] == false) then
			addundo({ "remove", unit.strings[UNITNAME], unit.values[XPOS], unit.values[YPOS], unit.values[DIR], unit.values[ID],
				unit.values[ID], unit.strings[U_LEVELFILE], unit.strings[U_LEVELNAME], unit.values[VISUALLEVEL],
			...
			if (editor.values[INEDITOR] == 0) then
				if (revert == false) then
					...
					if (levelconverts == 0) then
						...
						MF_store("save", generaldata.strings[WORLD] .. "_converts", tostring(totalconverts),
							generaldata.strings[WORLD] .. "_" .. upperlevel .. "_convert")
					...
					if (levelconverts > 0) then
						for a = 1, levelconverts do
							local result = string.find("___" ..
								MF_read("save", generaldata.strings[WORLD] .. "_" .. upperlevel .. "_convert", tostring(a - 1)),
								"___" .. generaldata.strings[CURRLEVEL])
					...
					MF_store("save", generaldata.strings[WORLD] .. "_" .. upperlevel .. "_convert", tostring(idtostore),
						generaldata.strings[CURRLEVEL] .. "," .. savestring)
					...
					if (levelconverts > 0) then
						for a = 1, levelconverts do
							...
							if (found > 0) and (a > found) then
								local newa = a - 1
								local datatostore = MF_read("save", generaldata.strings[WORLD] .. "_" .. upperlevel .. "_convert",
									tostring(a - 1))
					...
					if (found > 0) then
						...
						if (levelconverts > 0) then
							MF_store("save", generaldata.strings[WORLD] .. "_" .. upperlevel .. "_convert", "converts",
								tostring(levelconverts))
						else
							...
							if (totalconverts ~= nil) then
								for a = 1, totalconverts do
									local result = string.find("___" .. MF_read("save", generaldata.strings[WORLD] .. "_converts", tostring(a - 1))
										, "___" .. generaldata.strings[WORLD] .. "_" .. upperlevel .. "_convert")
