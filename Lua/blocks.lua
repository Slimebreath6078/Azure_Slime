function effectblock()
	...
	if (featureindex["level"] ~= nil) then
		...
		local colours = { isred, isorange, isyellow, islime, isgreen, iscyan, isblue, ispurple, ispink, isrosy, isblack, isgrey,
			issilver, iswhite, isbrown }
		local ccolours = { { 2, 2 }, { 2, 3 }, { 2, 4 }, { 5, 3 }, { 5, 2 }, { 1, 4 }, { 3, 2 }, { 3, 1 }, { 4, 1 }, { 4, 2 },
			{ 0, 4 }, { 0, 1 }, { 0, 2 }, { 0, 3 }, { 6, 1 } }
	...
	for i, unit in ipairs(units) do
		...
		if (unit.className ~= "level") then
			...
			local colours = { isred, isorange, isyellow, islime, isgreen, iscyan, isblue, ispurple, ispink, isrosy, isblack,
				isgrey, issilver, iswhite, isbrown }
			local ccolours = { { 2, 2 }, { 2, 3 }, { 2, 4 }, { 5, 3 }, { 5, 2 }, { 1, 4 }, { 3, 2 }, { 3, 1 }, { 4, 1 }, { 4, 2 },
				{ 0, 4 }, { 0, 1 }, { 0, 2 }, { 0, 3 }, { 6, 1 } }
			...
			if (#allfollows > 0) then
				for k, l in ipairs(allfollows) do
					if (issleep(l) == false) then
						...
						if (SB_is_group_of_text(unit) == true) then
							name = "text"
						end
							...
							if (paradox[uid] == nil) then
								...
								for b, bline in ipairs(currentundo) do
									...
									if (bline[1] == "remove") and (bline[6] == uid) then
										local x, y, dir, levelfile, levelname, vislevel, complete, visstyle, maplevel, colour, clearcolour, followed, back_init = bline
											[3], bline[4], bline[5], bline[8], bline[9], bline[10], bline[11], bline[12], bline[13], bline[14], bline[15]
											, bline[16], bline[17]
										...
										table.insert(delname,
											{ newunit.strings[UNITNAME], bline[6], newunit.values[XPOS], newunit.values[YPOS], newunit.values[DIR] })
									end
								end

								addundo({ "remove", unit.strings[UNITNAME], unit.values[XPOS], unit.values[YPOS], unit.values[DIR],
									unit.values[ID], unit.values[ID], unit.strings[U_LEVELFILE], unit.strings[U_LEVELNAME],
									unit.values[VISUALLEVEL
										], unit.values[COMPLETED], unit.values[VISUALSTYLE], unit.flags[MAPLEVEL], unit.strings[COLOUR],
									unit.strings[CLEARCOLOUR], unit.followed, unit.back_init, unit.originalname, unit.strings[UNITSIGNTEXT] })
								...
								if (below_[c] ~= 0) and (result ~= 1) then
									if (result ~= 0) then
										...
										for e, f in ipairs(specials) do
											...
											if (f[2] == "mine") then
												deletethese = SB_do_boom(deletethese, unitid)
												table.insert(deletethese, f[1])
												--End of Adds
											elseif (f[2] == "weak") then
												table.insert(deletethese, f[1])
											end
										end
									end
...
function block(small_)
		...
		if (generaldata2.values[ENDINGGOING] == 0) then
			...
			for i, unit in ipairs(doned) do
				...
				if (ufloat ~= 2) and (ded == false) then
					addundo({ "done", unit.strings[UNITNAME], unit.values[XPOS], unit.values[YPOS], unit.values[DIR], unit.values[ID],
						unit.fixed, ufloat, unit.originalname })
				end
		...
		if (unitmap[tileid] ~= nil) then
			...
			if (#water > 0) then
				for a, b in ipairs(water) do
					...
					if floating(b, unit.fixed, x, y) and
						(hasfeature(mmf.newObject(b).strings[UNITNAME], "is", "swim", unit.fixed, x, y) == nil) then
						--End of Changes
						if (b ~= unit.fixed) then
							local dosink = true
	...
	--Added by slimebreath6078
	delthese, removalshort, removalsound, doremovalsound = SB_execute_mine(removalshort, removalsound,
		doremovalsound, delthese)
						...
						for c, d in ipairs(skull) do
							...
							if (d ~= unit.fixed) then
								if floating(d, unit.fixed, x, y) then
									...
									--Added by slimebreath6078
									local mineskull = hasfeature(kname, "is", "mine", d)
									--Changed by slimebreath6078
									if ((weakskull == nil) and (mineskull == nil)) or (((weakskull ~= nil) or (mineskull ~= nil)) and issafe(d)) then
										--End of Add and Change
										doit = true
									end
								end
							else
		...
		for id, ugroup in ipairs(ismake) do
			...
			for a, unit in ipairs(ugroup[2]) do
				...
				if (ugroup[3] ~= "empty") then
					...
					name = getname(unit)
					leveldata = { unit.strings[U_LEVELFILE], unit.strings[U_LEVELNAME], unit.flags[MAPLEVEL], unit.values[VISUALLEVEL],
						unit.values[VISUALSTYLE], unit.values[COMPLETED], unit.strings[COLOUR], unit.strings[CLEARCOLOUR] }
				else
				...
				if exists then
					...
					if (name ~= "empty") then
						...
						if (#thingshere > 0) then
							for a, b in ipairs(thingshere) do
								...
								--print(thingname .. " == " .. v .. "?")
								if (thing.flags[CONVERTED] == false) and
									((thingname == v) or ((thing.strings[UNITTYPE] == "text") and (v == "text"))) then
									domake = false
								end
							end
						end
					...
					if domake then
						...
						elseif (v == "text") then
							if (SB_is_metatext(name) == false) and (name ~= "text") and (name ~= "all") then
			...
			if (win ~= nil) then
				for a, b in ipairs(win) do
					if (b[1] ~= "empty") then
						...
						if (#flag > 0) then
							for c, d in ipairs(flag) do
								if floating(d, unit.fixed, x, y) and (hasfeature(b[1], "is", "done", d, x, y) == nil) and
									(hasfeature(b[1], "is", "end", d, x, y) == nil) then
									local pmult = checkeffecthistory("win")
					...
					if (unitmap[tileid] == nil) or (#unitmap[tileid] == 0) then
						...
						for a, rules in ipairs(emptythings) do
							...
							if (rule[2] == "is") then
								...
								if canbonus and
									(
									(hasfeature("level", "is", "you", 1, i, j) ~= nil) or (hasfeature("level", "is", "you2", 1, i, j) ~= nil) or
										(hasfeature("level", "is", "3d", 1, i, j) ~= nil)) and floating_level(2, i, j) then
									bonus = true
								end
			...
			for i, rules in ipairs(things) do
				...
				if (rule[1] == "level") and (rule[2] == "is") and testcond(conds, 1) then
					...
					elseif (action == "defeat") then
						...
						--Changed by slimebreath6078
					elseif (action == "weak") or (action == "mine") then
						--End of Changes
						for i, unit in ipairs(units) do
							local name = unit.strings[UNITNAME]
							if (SB_is_group_of_text(unit) == true) then
					...
					elseif (action == "bonus") then
						...
						if (
							(#findallfeature("empty", "is", "you") > 0) or (#findallfeature("empty", "is", "you2") > 0) or
								(#findallfeature("empty", "is", "3d") > 0)) and floating_level(2) then
							bonusget = true
					...
					elseif (action == "win") then
						...
						if (hasfeature("level", "is", "you", 1) ~= nil) or (hasfeature("level", "is", "you2", 1) ~= nil) or
							(hasfeature("level", "is", "3d", 1) ~= nil) or emptyyou then
							canwin = true
						end
					...
					elseif (action == "fall") then
						...
						if (lstill == false) then
							addundo({ "levelupdate", Xoffset, Yoffset, Xoffset + tilesize * drop * ox, Yoffset + tilesize * drop * oy, dir,
								dir })
