...
function delete(unitid, x_, y_, total_, noinside_)
	...
	if (deleted[check] == nil) then
		...
		if (unitid ~= 2) then
			...
			addundo({ "remove", unitname, x, y, dir, unit.values[ID], unit.values[ID], unit.strings[U_LEVELFILE],
				unit.strings[U_LEVELNAME], unit.values[VISUALLEVEL], unit.values[COMPLETED], unit.values[VISUALSTYLE],
				unit.flags[MAPLEVEL], unit.strings[COLOUR], unit.strings[CLEARCOLOUR], unit.followed, unit.back_init,
				unit.originalname, unit.strings[UNITSIGNTEXT] }, unitid)
			unit = {}
			delunit(unitid)
			MF_remove(unitid)
...
function writerules(parent, name, x_, y_)
	...
	for i, rules in ipairs(visualfeatures) do
		...
		local target_noun, verb, object = SB_convert_visual_option(rule)

		if (#custom == 0) then
			text = text .. target_noun .. " "
		else
			text = text .. custom .. " "
		end
		...
		if (fullinvis == false) then
			if (#conds > 0) then
				for a, cond in ipairs(conds) do
					...
					if middlecond then
						if (#custom == 0) then
							local target = SB_convert_name(cond[1])
						...
						if (cond[2] ~= nil) then
							if (#cond[2] > 0) then
								for c, d in ipairs(cond[2]) do
									if (#custom == 0) then
										local target = SB_convert_name(d)
...
function copy(unitid, x, y)
	for i, unit in ipairs(units) do
		if (unit.fixed == unitid) then
			local oldx, oldy, dir, name, float = unit.values[XPOS], unit.values[YPOS], unit.values[DIR], unit.strings[UNITNAME],
				unit.values[FLOAT]

			local leveldata = { unit.strings[U_LEVELFILE], unit.strings[U_LEVELNAME], unit.flags[MAPLEVEL],
				unit.values[VISUALLEVEL], unit.values[VISUALSTYLE], unit.values[COMPLETED], unit.strings[COLOUR],
...
function inside(name, x, y, dir_, unitid, leveldata_)
	...
	if (#ins > 0) then
		for i, v in ipairs(ins) do
			local object = v[1]
			local conds = v[2]
			--print(object .. ", " .. tostring(testcond(conds, unitid, x, y)))
			if testcond(conds, unitid, x, y) then
				if (object ~= "text") then
					if (SB_is_metatext(object) == false) then
...
function animate()
	...
	for i, unitid in ipairs(animunits) do
		...
		if (spritedata.values[VISION] == 1) and (unit.values[ID] == spritedata.values[CAMTARGET]) and
			(unit.values[ZLAYER] < 21) then
			camtarget = true
		end
...
function issolid(unitid)
	...
	print(name .. " move")
	local isyou = hasfeature(name, "is", "you", unitid) or hasfeature(name, "is", "you2", unitid) or
		hasfeature(name, "is", "3d", unitid)
...
function isgone(unitid)
	if (issafe(unitid) == false) then
		...
		local isyou = hasfeature(name, "is", "you", unitid, x, y) or hasfeature(name, "is", "you2", unitid, x, y) or
			hasfeature(name, "is", "3d", unitid, x, y)
		...
		if (issink ~= nil) then
			for i, v in ipairs(issink) do
				...
				if (v ~= unitid) and floating(v, unitid, x, y) and (hasfeature(name, "is", "swim", unitid, x, y) == false) then
					--End of Changes
					return true
				end
			end
			...
			if (rule[1] == "empty") then
				...
				elseif (rule[2] == "follow") and (rule[3] ~= "all") and (rule[3] ~= "empty") and (rule[3] ~= "level") then
					...
					for c, name in ipairs(checkthese) do
						if (followcheck == false) and (issleep(2, x, y) == false) and (objectlist[name] ~= nil) and
							(unitlists[name] ~= nil) then
...
function getname(unit, meta_)
	...
	if (meta == false) and (SB_is_group_of_text(unit) == true) then
		result = "text"
	end
...
function isitbroken(name, id, x, y, checkedconds)
	if (hasfeature(name, "is", "broken", id, x, y, checkedconds, true) ~= nil) then
		return 1
	end
...
function hasconds(rule)
	...
	if (conds == nil) then
		...
		if (type(conds) ~= "table") then
			MF_alert("Warning! Conditionals of " ..
				rule[1][1] .. " " .. rule[1][2] .. " " .. rule[1][3] .. " aren't in a table!")
			return false
		elseif (#conds == 0) then
			return false
...
function simplecheck(x, y, noempty_, checkc)
	...
	if (#obs > 0) then
		for i, id in ipairs(obs) do
			if (id == -1) then
				...
				if (
					(hasfeature(obsname, "is", "stop", id, x, y, checkc) ~= nil) or
						(hasfeature(obsname, "is", "push", id, x, y, checkc) ~= nil) or
						(hasfeature(obsname, "is", "pull", id, x, y, checkc) ~= nil)) and
					(hasfeature(obsname, "is", "hide", id, x, y, checkc) == nil) and
					(hasfeature(obsname, "is", "phantom", id, x, y, checkc) == nil) then
					return 1
				end
			end
		end
	elseif (noempty == false) then
		if (
			(hasfeature("empty", "is", "stop", 2, x, y, checkc) ~= nil) or
				(hasfeature("empty", "is", "push", 2, x, y, checkc) ~=
					nil) or (hasfeature("empty", "is", "pull", 2, x, y, checkc) ~= nil)) and
			(hasfeature("empty", "is", "phantom", 2, x, y, checkc) == nil) and
			(hasfeature("empty", "is", "hide", 2, x, y, checkc) == nil) then
			return 2
		end
	end
