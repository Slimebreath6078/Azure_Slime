function formlettermap()
	...
	if (#letterunits > 0) then
		...
		for tileid, v in pairs(lettermap) do
			...
			if (#cobjects > 0) then
				--print("--finding custom object")
				for a, b in ipairs(v) do
					local n = b[1]
					--print(n)
					if (cobjects[n] ~= nil) then
						--print("Found custom object " .. n)
						continuer = true
						continued = true
						break
					end
		...
		for i, v in ipairs(letterunitlist) do
			...
			--print("In database: " .. v[1] .. ", dir " .. tostring(v[5]))
...
function cullnotes(database_)
	...
	for i, v in ipairs(database) do
		...
		if (freqs[w] ~= nil) or (specialcases[sw] ~= nil) then
			...
			print("Adding " ..
				w ..
				", dir " .. tostring(v[5]) .. " to " .. tostring(tileid) .. ", longest " .. tostring(comparison[tileid]["longest"]))
			]]
	...
	for j = 1, #delthese do
		...
		--print("Deleting " .. tostring(c) .. ", " .. database[c][1])
		...
		for j = 1, totalcombos do
			...
			for i, cjumble in ipairs(jumble) do
				...
				print(tostring(j) ..
					...
					", " ..
					cword[1] ..
					", " .. subword .. ", " .. prevword .. ", " .. tostring(dir) .. ", " .. tostring(found) .. ", " .. jumblecombo[i])
				]]
					...
					if (#fullwords > 0) then
						for a, b in ipairs(fullwords) do
							...
							if (been_seen[wordcode] == nil) then
								...
								--print("Adding to database: " .. w .. ", " .. tostring(dir) .. ", " .. wordcode)
