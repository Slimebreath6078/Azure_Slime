		...
		if (#conds > 0) then
			...
			for i, cond in ipairs(conds) do
				...
				if (condlist[basecondtype] ~= nil) then
					...
					local ccc = false
					local cdata = { name = name, x = x, y = y, unitid = unitid, dir = dir, extras = extras, limit = limit, conds = conds,
						subtype = condsubtype, i = i, surrounds = surrounds, notcond = notcond, debugname = cond[1] }
...
condlist =
{
	...
	on = function(params, checkedconds, checkedconds_, cdata)
		...
		--print(mmf.newObject(unitid).strings[UNITNAME])
							...
							if (unitmap[tileid] ~= nil) then
								for c, d in ipairs(unitmap[tileid]) do
									if (d ~= unitid) and (alreadyfound[d] == nil) then
										...
										if (SB_is_targetof_condition(pname, d, pnot) == true) and (alreadyfound[bcode] == nil) then
								...
								print("unitmap is nil at " ..
									tostring(x) .. ", " .. tostring(y) .. " for object " .. tostring(name) .. " (" .. tostring(unitid) .. ")!")
							end
	...
	facing = function(params, checkedconds, checkedconds_, cdata)
		...
		local unitid, x, y, dir, extras, surrounds, conds = cdata.unitid, cdata.x, cdata.y, cdata.dir, cdata.extras,
			cdata.surrounds, tostring(cdata.conds)
				...
				if (unitid ~= 1) then
					if ((pname ~= "empty") and (b ~= "level")) or ((b == "level") and (alreadyfound[1] ~= nil)) then
						if (stringintable(pname, extras) == false) then
							...
							if (pnot == false) then
								if ((pname == "right") and (dir == 0)) or ((pname == "up") and (dir == 1)) or ((pname == "left") and (dir == 2))
									or ((pname == "down") and (dir == 3)) then
	...
	seeing = function(params, checkedconds, checkedconds_, cdata)
		...
		local unitid, x, y, dir, conds, surrounds = cdata.unitid, cdata.x, cdata.y, cdata.dir, tostring(cdata.conds),
			cdata.surrounds
					...
					if (unitmap[tileid] ~= nil) then
						if (#unitmap[tileid] > 0) then
							...
							for a, b in ipairs(unitmap[tileid]) do
								...
								--Changed by slimebreath6078--
								local name_ = (SB_is_metatext(unit.strings[UNITNAME]) == true) and unit.strings[UNITNAME] or getname(unit)
								local is_hide = (
									(SB_is_metatext(unit.strings[UNITNAME]) == true) and
										(
										(hasfeature(name_, "is", "hide", b, nx, ny, checkedconds) ~= nil) or
											(hasfeature(unit.strings[UNITNAME], "is", "hide", b, nx, ny, checkedconds) ~= nil))) or
									(
									(SB_is_metatext(unit.strings[UNITNAME]) == false) and
										(hasfeature(name_, "is", "hide", b, nx, ny, checkedconds) ~= nil))
								--End of Change--
