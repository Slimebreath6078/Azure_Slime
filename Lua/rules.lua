					...
					if (alreadyused[tileid] == nil) and (unit.values[TYPE] ~= 5) and (unit.flags[DEAD] == false) then
						for i = 1, 2 do
							...
							if (#hm == 0) and (#hm2 > 0) then
								--print("Added " .. unit.strings[UNITNAME] .. " to firstwords, dir " .. tostring(i))
	...
	if (#firstwords > 0) then
		for k, unitdata in ipairs(firstwords) do
			...
			print("Testing " ..
				word ..
				": " ..
				tostring(donefirstwords[unique_id]) ..
				", " .. tostring(dir) .. ", " .. tostring(unitid) .. ", " .. tostring(unique_id))
			]]
			...
			if (donefirstwords[unique_id] == nil) or
				((donefirstwords[unique_id] ~= nil) and (donefirstwords[unique_id][dir] == nil)) and (limiter < 5000) then
				local ox, oy = 0, 0
				local name = word
				...
				if (maxlen > 2) then
					for i = 1, variations do
						...
						for wordid = existing_wordid, #sent do
							...
							-- Add by Slimebreath6078 --
							if (stage == 7) then
								--print("converted textof")
							...
							--[[
								...
								8 = customobject
								-- Add by Slimebreath6078 --
								9 = textof(yksi eritysista alkusanoista)
							]] --
							...
							--[[
								...
								1 = after a foreword
								2 = after a noun
								3 = after verb or condition word
								4 = after AND by a noun or property
								5 = after a target
								6 = after AND by a foreword
								-- Add by Slimebreath6078 --
								7 = after a textof
								8 = after a letter by a textof
								9 = agter a letter by a textof and target
							]]
							if (tiletype ~= 5) then
								if (stage == 0) then
									--print("firstword = " .. unitdata[4])
									if (tiletype == 0) then
										prevstage = stage
										stage = 2
								...
								elseif (stage == 2) then
									if (wordid ~= #sent) then
										if (tiletype == 1) and (prevtiletype ~= 4) and ((prevstage ~= 4) or doingcond or (stage3reached == false)) then
											...
											stage = 3
										elseif (tiletype == 7) and (stage2reached == false) and (nocondsafterthis == false) and
											((doingcond == false) or (prevstage ~= 4)) then
											doingcond = true
											prevstage = stage
											stage = 3
								...
								elseif (stage == 4) then
									if (wordid <= #sent) then
										...
										elseif ((tiletype == 1) and stage3reached) and (doingcond == false) and (prevtiletype ~= 4) then
											...
											prevstage = stage
											stage = 3
										elseif (tiletype == 7) and (nocondsafterthis == false) and
											((prevtiletype ~= 6) or ((prevtiletype == 6) and doingcond)) then
											doingcond = true
											stage2reached = true
											prevstage = stage
											stage = 3
							...
							--print(tilename .. ", " .. prevstage .. " -> " .. stage)
							...
							if (tiletype == 4) then
								...
								--Changed by slimebreath6078
								if (stop == false) and (tiletype ~= 0) and (tiletype ~= 9) then
									--End of changes
									notids = {}
									notwidth = 0
									notslot = 0
							...
							--[[print(tilename ..
								...
								", " ..
								tostring(tiletype) ..
								", " .. tostring(prevtiletype) .. ", " .. tostring(stop) .. ", " .. name .. ", " .. tostring(i))]] --
							...
							if (stop == false) then
								if (stage ~= 7) then
									...
									if (wordid == #sent) and (#current >= 3) and (j > 1) then
										...
										--print("Checking finals: " .. subsent_id .. ", " .. tostring(existingfinals[subsent_id]))
								...
								if ((tiletype == 0) or (tiletype == 9)) and (prevtiletype == 0) and (#notids > 0) then
									notids = {}
									notwidth = 0
								end
								...
								--print(wordid .. ", " .. #sent .. ", " .. existing_wordid .. ", " .. tostring(firstrealword))

								if (wordid < #sent) then
									if (wordid > existing_wordid) then
										if (#notids > 0) and firstrealword and (notslot > 1) and
											--Changed by slimebreath6078
											((tiletype ~= 1) or ((tiletype == 1) and ((prevtiletype == 0) or (prevtiletype == 9)))) then
											--End of changes
											--print(tostring(notslot) .. ", not -> A, " .. unique_id .. ", " .. sent_id)
											...
											if (nexts[2] ~= nil) and ((nexts[2] == 0) or (nexts[2] == 3) or (nexts[2] == 4)) and (tiletype ~= 3) then
												local insert_wordid = wordid
												--print(tostring(wordid) .. ", " .. tilename .. " -> B, " .. unique_id .. ", " .. sent_id)
												subsent_id = string.sub(sent_id, j)
												table.insert(firstwords, { s[3], dir, tilewidth, tilename, tiletype, sent, insert_wordid, subsent_id })
											end
										else
											if ((prevtiletype == 0) or (prevtiletype == 9)) and (tiletype == 1) then
												local insert_wordid = wordid
												--[[
												print(tostring(wordid - 1) ..
													", " .. tilename .. ", " .. sent[wordid - 1][1] .. " -> C, " .. unique_id .. ", " .. sent_id)
												]]
												--print(sent[wordid - 2][1])
												if (sent[wordid - 2][1] == "textof") then
													insert_wordid = insert_wordid - 1
												end
												local subsent_id = string.sub(sent_id, insert_wordid - existing_wordid)
												table.insert(firstwords,
													{ sent[wordid - 1][3], dir, tilewidth, tilename, tiletype, sent, insert_wordid - 1, subsent_id })
											elseif ((prevsafewordtype == 0) or (prevsafewordtype == 9)) and (prevsafewordid > 0) and (prevtiletype == 4)
												and (tiletype ~= 1) and
												(tiletype ~= 2) then
												--[[
												print(tostring(prevsafewordid) ..
													", " .. sent[prevsafewordid][1] .. " -> D, " .. unique_id .. ", " .. sent_id)
													]]
												local subsent_id = string.sub(sent_id, (prevsafewordid - existing_wordid) + 1)
												table.insert(firstwords,
													{ sent[prevsafewordid][3], dir, tilewidth, tilename, tiletype, sent, prevsafewordid, subsent_id })
											else
												--print(tostring(wordid) .. ", " .. tilename .. " -> E, " .. unique_id .. ", " .. sent_id)
									...
									elseif (wordid == existing_wordid) then
										if (nexts[3][1] ~= -1) then
											--print(tostring(wordid + 1) .. ", " .. nexts[1] .. " -> F, " .. unique_id .. ", " .. sent_id)
									...
									if (#b == #sentence) and (a > i) then
										...
										if identical then
											--[[
												...
												" (" ..
												tostring(i) ..
												") is identical to " .. b[1][1] .. ", " .. b[2][1] .. ", " .. b[3][1] .. " (" .. tostring(a) .. ")")
												]] --
											valid = false
										end
									end
						...
						if valid then
							for index, wdata in ipairs(sentence) do
								...
								if (wcategory == 0) then
									local allowed = false
									--Added by slimebreath6078
									--print("(docode) " .. prev_wname .. ", " .. wname)
									if (prev_wname == "play") then
										for i_dummy, v in ipairs(changes[unitreference["text_play"]].customobjects) do
											if (wname == v) then
												allowed = true
												break
											end
										end
									elseif ((prev_wname ~= "write")) or (SB_is_metatext(wname) == false) then
										--End of Add
								...
								elseif (wcategory == 1) then
									if (index < #sentence) then
										...
										--print(wname .. ", " .. tostring(realname) .. ", " .. "text_" .. wname)
...
function codecheck(unitid, ox, oy, cdir_, ignore_end_, wordunitresult_)
	...
	if (unitmap[tileid] ~= nil) then
		for i, b in ipairs(unitmap[tileid]) do
			...
			if (v.values[TYPE] ~= 5) and (v.flags[DEAD] == false) then
				...
				--print("(letter) " .. v.strings[UNITNAME])
				justletters = true
			end
		end
	end
	...
	if (letterunits_map[tileid] ~= nil) then
		for i, v in ipairs(letterunits_map[tileid]) do
			...
			--print(word .. ", " .. tostring(valid) .. ", " .. tostring(dir) .. ", " .. tostring(cdir))
...
function addoption(option, conds_, ids, visible, notrule, tags_)
	...
	if (#option == 3) then
		...
		if (string.sub(target, 1, 5) == "group") or (string.sub(effect, 1, 5) == "group") or
			(string.sub(target, 1, 9) == "not group") or (string.sub(effect, 1, 9) == "not group") then
			groupcond = true
		end
					...
					if (#cond[2] > 0) then
						...
						for a, b in ipairs(cond[2]) do
							...
							elseif (b == "not all") then
								...
								--Add by slimebreath6078--
							elseif (string.sub(b, 1, 9) == "not text_") then
								--print("-- not textof " .. string.sub(b, 10) .. " --")
								local inserted_text = {}
								for i, v in ipairs(codeunits) do
									local temp_unit = mmf.newObject(v)
									if (temp_unit.strings[UNITNAME] ~= string.sub(b, 5)) then
										inserted_text[temp_unit.strings[UNITNAME]] = 1
									end
								end
								for text, i in pairs(inserted_text) do
									--print("inserted " .. text)
									table.insert(newconds, text)
								end
								--End of Add
		...
		if (targetnot == "not ") and ((objectlist[targetnot_] ~= nil) or (SB_is_metatext(targetnot_) == true)) and
			(string.sub(targetnot_, 1, 5) ~= "group") and
			(string.sub(effect, 1, 5) ~= "group") and (string.sub(effect, 1, 9) ~= "not group") or
			(((string.sub(effect, 1, 5) == "group") or (string.sub(effect, 1, 9) == "not group")) and (targetnot_ == "all")) then
			if (targetnot_ ~= "all") and (SB_is_metatext(targetnot_) == false) then
				...
				--Add by slimebreath6078--
			elseif (SB_is_metatext(targetnot_) == true) then
				--print("-- not textof " .. string.sub(targetnot_, 6) .. " --")
				local inserted_text = {}
				for i, v in ipairs(codeunits) do
					local temp_unit = mmf.newObject(v)
					if (temp_unit.strings[UNITNAME] ~= targetnot_) then
						inserted_text[temp_unit.strings[UNITNAME]] = 1
					end
				end

				for text, i in pairs(inserted_text) do
					local rule = { text, verb, effect }
					local newconds = {}
					--print("inserted " .. text)
					for a, b in ipairs(conds) do
						table.insert(newconds, b)
...
function subrules()
	...
	if (featureindex["text"] ~= nil) then
		for k, rules in ipairs(featureindex["text"]) do
			...
			if (rule[1] == "text") then
				for name, dummy in pairs(nocodeunits) do
					--print("not group of text: " .. name)
	...
	if (featureindex["mimic"] ~= nil) then
		for i, rules in ipairs(featureindex["mimic"]) do
			...
			if (rule[2] == "mimic") then
				...
				if valid then
					--Changed by slimebreath6078
					if ((SB_is_metatext(object) == true) or (getmat(object) ~= nil)) and
						((SB_is_metatext(target) == true) or (getmat(target) ~= nil)) then
						--End of changes
						if (featureindex[target] ~= nil) then
							--print(target .. " -> copythese " .. #featureindex[target])
							copythese = featureindex[target]
						end
						--Added by slimebreath6078
						if (SB_is_metatext(target) == true) then
							for i_dummy, v in ipairs(featureindex["text"]) do
								table.insert(copythese, v)
							end
						end
						--End of Add
					end
					...
					for a, b in ipairs(copythese) do
						...
						--Changed by slimebreath6078
						if ((trule[1] == target) or ((SB_is_metatext(target) == true) and (trule[1] == "text"))) and (trule[2] ~= "mimic")
							and valid then
							--End of changes
							--print(trule[1] .. ", " .. trule[3])
							local newconds = {}
							local newtags = {}
...
function postrules(alreadyrun_)
	...
	for i, rules in ipairs(features) do
		print(rules[1][1], rules[1][2], rules[1][3])
		if (i <= limit) then
			local rule = rules[1]
			local conds = rules[2]
			local ids = rules[3]
			...
			elseif (SB_text_protected(rule) == true) then
				table.insert(protects, i)
			end
					...
					for a, d in ipairs(idlist) do
						for c, b in ipairs(d) do
							if (b ~= 0) then
								...
								if (ruleids[b] == nil) and (#undobuffer > 1) and (alreadyrun == false) and
									(generaldata5.values[LEVEL_DISABLERULEEFFECT] == 0) then
			...
			if (rulenot == 1) then
				...
				for e, g in ipairs(targetlists) do
					for a, b in ipairs(featureindex[g]) do
						...
						if same or ((g == "write") and (target == b[1][1]) and (b[1][2] == "write")) or
							(
							(SB_is_metatext(neweffect) == true) and (target == b[1][1]) and (b[1][2] == "write") and
								(b[1][3] == string.sub(neweffect, 6))) then
							--print(rule[1] .. ", " .. rule[2] .. ", " .. neweffect .. ": " .. b[1][1] .. ", " .. b[1][2] .. ", " .. b[1][3])
							...
							--[[
							for i_dummy, v in ipairs(theseconds) do
								print(v[1])
							end
							]]
	...
	--Add by slimebreath6078
	local pures = SB_create_pure_list()
	if (#pures > 0) then
		SB_execute_pures(pures)
	end
	--Endof Adds
			...
			if (conds[1] ~= "never") then
				...
				if (featureindex[target] ~= nil) then
					for a, rules in ipairs(featureindex[target]) do
						...
						--print("(protect)" .. targetrule[1] .. " " .. targetrule[2] .. " " .. object)

						if (targetrule[1] == target) and
							(
							((string.sub(target, 0, 5) ~= "text_") and (targetrule[2] == "is") and (target ~= object)) or
								((targetrule[2] == "write") and (string.sub(object, 1, 4) ~= "not ")) or
								(SB_can_convert_text(target, targetrule[2], object) == true)) and
							(
							((getmat(object) ~= nil) or (SB_is_metatext(object) == true)) or (object == "revert") or
								((targetrule[2] == "write") and (string.sub(object, 1, 4) ~= "not "))) and (string.sub(object, 1, 5) ~= "group") then
							if (#newconds > 0) then
								if (newconds[1] == "never") then
									targetconds = {}
								end
		...
		while (diffname_done == false) and (evilrec_limit < 10) do
			...
			for i, v_ in ipairs(notgroupisgroup_diffname) do
				...
				for a, b in pairs(objectlist) do
					if (findnoun(a) == false) and
						(
						(evilrec_memberships_quick[gn1] == nil) or
							((evilrec_memberships_quick[gn1] ~= nil) and (evilrec_memberships_quick[gn1][a] == nil))) then
...
function calculatesentences(unitid, x, y, dir)
		...
		if (totalvariants < limiter) then
			...
			if (#words > 0) then
				...
				for i, v in ipairs(words) do
					...
					--print("Step " .. tostring(step) .. ", word " .. v[3] .. " here, " .. tostring(v[2]))
					--print("textof " .. tostring(textof))
					...
					--Add by Slimebreath6078--
					if starting and ((v[4] == 0) or (v[4] == 3) or (v[4] == 4) or (v[4] == 9)) then
						starting = false
					end
					if (prevsharedtype == 9) then
						objfound = true
						textof = true
					end
					-- End of add
				...
				if (sharedtype >= 0) and (prevsharedtype >= 0) and (#words > 0) and (maxw == 1) and (prevmaxw == 1) and (currw == 0) then
					if ((sharedtype == 0) and (prevsharedtype == 0)) or
						--Add by Slimebreath6078--
						((sharedtype == 1) and (prevsharedtype == 1) and (textof == false)) or
						-- End of add
						((sharedtype == 2) and (prevsharedtype == 2)) or
						((sharedtype == 0) and (prevsharedtype == 2)) then
						done = true
						sents[step] = nil
						--[[
						print("added " ..
							tostring(words[1][1]) ..
							" " .. tostring(words[1][2]) .. " " .. tostring(words[1][3]) .. " " .. tostring(words[1][4]) .. " ")
							]]
				...
				print("Step " ..
					tostring(step) ..
					", no words here, " ..
					tostring(letters) .. ", " .. tostring(jletters) .. ", " .. mmf.newObject(unitid).strings[UNITNAME])
				]]
	...
	--print(tostring(step) .. ", " .. tostring(totalvariants))
		...
		while (step < maxpos) do
			...
			if (c ~= nil) then
				...
				if (c > 0) then
					...
					--[[
						print(tostring(i) ..
						", step " .. tostring(step) .. ": " .. word[3] .. ", " .. tostring(#word[1]) .. ", " .. tostring(w))
						]]
